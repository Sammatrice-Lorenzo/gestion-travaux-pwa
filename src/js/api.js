...
function fetchFileAPI(routeDTO, nameFile) {
    ...
    fetch(routeDTO.getUrlAPI(), {
        ...
        if (response.status === 200) {
            ...
            $f7.dialog.alert(response.status === 401 ? messages.TOKEN_EXPIRED : messages.ERROR_SERVER)
        }
    }
    )
        .catch(async (error) => {
    ...
    await fetch(url, {
        ...
        response
            ...
            .then(async (data) => {
                ...
                let dataResponse = data.hasOwnProperty('hydra:member') ? data["hydra:member"] : data
                dataResponse = Array.isArray(dataResponse) ? dataResponse : [dataResponse]
