 ...
 */
const handleDeleteProductInvoice = async (
  element,
  framework7DTO,
  ...
  return $f7.dialog.confirm(CONFIRMATION_TO_DELETE, async () => {
    await deleteProductInvoice($f7, invoiceId)

    productInvoicesByUser = productInvoicesByUser.filter(
      (product) => product.id !== invoiceId,
    )
...
const downloadPDF = (element, framework7DTO, productInvoicesByUser) => {
  ...
  const invoiceId = getIdOfElementClicked(framework7DTO.getSelector(), element)
  const productInvoice = productInvoicesByUser.filter(
    (productInvoice) => productInvoice.id === Number.parseInt(invoiceId),
  )[0]
