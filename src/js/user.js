  ...
  fetch(url, {
    ...
    .then((response) =>
      response.json().then((data) => {
        if (data.code === 422) {
          $f7.dialog.alert(
            "Cette email existe déjà au sein de l'application veuillez ressayer avec une autre",
          )
        } else {
          $f7.dialog.alert(messages.SUCCESS_INSERTION_FORM)
          $f7.dialog.alert(
            'Un email de confirmation vous a été envoyée veuillez confirmer votre email avant de vous connecté',
          )
